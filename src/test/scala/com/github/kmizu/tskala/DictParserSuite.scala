...
class DictParserSuite extends FunSuite {
  ...
  test("parse dictionary in typed program") {
    val code = """
      function getValue(dict: Dict[String, Int], key: String): Int {
        dict
      }
    ...
    assertEquals(result.functions.head.params.length, 2)
    assertEquals(result.functions.head.params(0)._2, Type.TDict(Type.TString, Type.TInt))
  }
  ...
  test("parse Dict type in function parameter") {
    val code = """
      function processDict(d: Dict[Int, String]): Int {
        size(d)
      }
    """
    val result = parseProgram(code)
    val func = result.functions.head
    assertEquals(func.params.head._2, Type.TDict(Type.TInt, Type.TString))
  }

  test("parse Dict type with nested types") {
    val code = """
      function nestedDict(d: Dict[String, List[Int]]): List[Int] {
        values(d)
      }
    """
    val result = parseProgram(code)
    val func = result.functions.head
    assertEquals(func.params.head._2, Type.TDict(Type.TString, Type.TList(Type.TInt)))
  }
}