...
class DictEvaluatorSuite extends FunSuite {
  ...
  test("evaluate dictionary with list values") {
    val dict = tDict(
      ...
      (tString("names"), tList(tString("a"), tString("b")))
    )
  ...
  test("evaluate nested dictionaries") {
    ...
    val outerDict = tDict(
      (tString("inner"), innerDict),
      (tString("value"), tInt(20))
    )
    val result = evalExp(tDictAccess(tDictAccess(outerDict, tString("inner")), tString("x")))
